...
class LocationPicker extends React.Component {
  fetchEpisodes = () => {
    ...
    fetch(
      `http://localhost:3001/episodes?lat=${latitude}&lng=${longitude}&start=${Date.now()}`
    )
      ...
      <div className="geo-container">
        ...
        <form className="horizontal-group">
          <button
            ...
            onClick={this.getCoordsFromUserAgent}>
            <img
              alt="geolocation"
              className="geolocate-icon"
...
const mapDispatchToProps = dispatch => ({
  changeCoords: (latitude, longitude) =>
    dispatch(changeCoords(latitude, longitude)),
